...
module.exports = function pino(options = {}) {
  ...
  return pinoms(Object.assign({}, options, {
    ...
    streams: [
      console && {
        level: console.level ? console.level.toLowerCase() : 'trace',
        stream: getPrettyStream(Object.assign({
          translateTime: 'SYS:standard',
          ignore: 'project',
        }, console), undefined, process.stdout),
      },
      udp && {
        level: udp.level ? udp.level.toLowerCase() : 'trace',
        stream: new Writable({
          close() {
            socket.close();
          },
